...
def call_llm_vision(
    ...
    if not all([model, provider, system_prompt, user_prompt]):
        raise ValueError("Missing required LLM vision parameters")
    response, actual_provider, actual_model = get_llm().call_with_fallback(
        model=model,
        provider=provider,
        system_prompt=system_prompt,
    ...
    for current_provider, current_model in attempts:
        try:
            if current_provider == "groq":
                start = time.perf_counter()
                result = get_llm()._call_groq_audio(audio_bytes, audio_mime_type, current_model)
                duration_ms = round((time.perf_counter() - start) * 1000, 2)
                logger.info(
                    "Audio transcription success | provider=%s model=%s duration_ms=%s",
                    current_provider, current_model, duration_ms,
                )
                return result
            raise ValueError(f"Audio transcription not supported for provider '{current_provider}'")
        except Exception as e:
            errors.append(f"{current_provider}/{current_model}: {repr(e)}")
            logger.warning(
                "Audio transcription failed [%s/%s]: %s",
                current_provider, current_model, repr(e),
            )

    raise RuntimeError(
        "All audio transcription providers failed. Attempts: " + " | ".join(errors)
    )
