...
export function GlobalSearchBar() {
  ...
  function toggleSmartSearch() {
    ...
    setStatus('idle')
    setError(null)
    if (query.trim() && !smartSearch) {
      // switching TO smart — clear auto results
    }
  }
  ...
  return (
    <div ref={containerRef} className="relative">
      <form onSubmit={handleSubmit} className="flex items-center gap-1.5">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="h-8 w-56 rounded-lg border border-[#e2e8f0] bg-[#f8fafc] pl-8 pr-6 text-[12px] text-navy outline-none transition-colors focus:border-teal focus:ring-2 focus:ring-teal/20 placeholder:text-[#9ca3af]"
          />
          {query && (
            <button
              type="button"
              onClick={clear}
              className="absolute right-2 top-1/2 -translate-y-1/2 text-[#9ca3af] hover:text-navy"
            >
              <X className="size-3" />
            </button>
          )}
        ...
        <button
          ...
          title={
            smartSearch
              ? 'Smart Search ON — searching by meaning (AI)'
              : 'Smart Search OFF — click to enable AI-powered semantic search'
          }
        ...
        {/* Submit — only visible in smart mode */}
        {smartSearch && (
          <button
            type="submit"
      ...
      {/* Results dropdown */}
      {showDropdown && (
        <div className="absolute right-0 top-full mt-2 w-[500px] rounded-xl border border-[#e2e8f0] bg-white shadow-xl z-50">
          {/* Dropdown header */}
            ...
            <div className="flex items-center gap-1.5">
              {smartSearch ? (
                <>
                  ...
                  <span className="text-[11px] font-semibold text-navy">Smart Search</span>
                  <span className="rounded-full bg-teal/10 px-1.5 py-0.5 text-[9px] font-semibold text-teal">AI</span>
                  <span className="ml-1 text-[11px] text-[#6b7280]">Searching case notes by meaning</span>
                </>
              ) : (
                <>
                  <Search className="size-3.5 text-[#9ca3af]" />
                  <span className="text-[11px] font-semibold text-navy">Clients</span>
                </>
              )}
            </div>
            {hasResults && (
          ...
          <div className="max-h-[440px] overflow-y-auto p-2">
            ...
            {status === 'error' && (
              <div className="flex items-start gap-2 rounded-lg border border-amber-200 bg-amber-50 mx-1 my-2 px-3 py-2.5">
                ...
                <p className="text-[12px] text-amber-700">{error ?? 'Something went wrong. Please try again.'}</p>
              </div>
            )}

            {/* Smart search — idle hint */}
              ...
              clientResults.map(c => (
                ...
                >
                  <div className="flex items-center gap-2 min-w-0">
                    <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-navy/10 text-[11px] font-semibold text-navy">
                    ...
                    <div>
                      <Link
                        ...
                        className="block text-[13px] font-semibold text-navy hover:text-teal hover:underline leading-tight"
                      >
                        {c.first_name} {c.last_name}
                      </Link>
                      <span className="text-[11px] text-[#9ca3af]">{c.client_number}</span>
                    </div>
                  </div>
                  <Link
                    ...
                    <div className="flex items-start gap-3">
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          <span className="text-[10px] text-[#9ca3af]">{r.client_number}</span>
                        ...
                        {/* Note snippet */}
                        {snippet && (
                          <p className="mt-1.5 text-[12px] text-[#374151] leading-relaxed line-clamp-2">
